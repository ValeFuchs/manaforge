...
Valence then ported this code from SkyRat
...
/mob/living/simple_animal/hostile/megafauna/sif
	...
	maxHealth = 2000
	attacktext = "slashes"
	attack_sound = 'sound/lavaland/sif_slash.ogg'
	...
	death_sound = 'sound/lavaland/death_howl.ogg'
...
/mob/living/simple_animal/hostile/megafauna/sif/proc/angered()
	...
	playsound(src, 'sound/lavaland/howl.ogg', 100, 1)
	...
	if(can_special == 1)
		...
		if(spinning == TRUE)
			...
			if(isturf(src.loc) || isobj(src.loc) && src.loc.density)
				...
				playsound(src, pick('sound/lavaland/whoosh1.ogg', 'sound/lavaland/whoosh2.ogg', 'sound/lavaland/whoosh3.ogg'), 300, 1)
				playsound(src, 'sound/lavaland/blade_spin.ogg', 400, 1)
				if(angered)
					src.speed = 8
					src.move_to_delay = 2
...
/mob/living/simple_animal/hostile/megafauna/sif/Bump(atom/A)
	if(charging == TRUE)
		...
		if(isliving(A))
			...
			playsound(get_turf(L), 'sound/lavaland/sif_stomp.ogg', 400, 1)
			shake_camera(L, 4, 3)
